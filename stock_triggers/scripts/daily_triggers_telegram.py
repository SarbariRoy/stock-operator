...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--alerts-json",
        type=str,
        default=str(DATA_DIR / "pipeline_alerts.json"),
        help="Path to pipeline alerts JSON generated by run_signal_refresh_pipeline.py",
    )
...
def build_message(
    *,
    ...
    if isinstance(alerts_payload, dict) and alerts_payload:
        runtime_seconds = int(alerts_payload.get("runtime_seconds", 0) or 0)
        runtime_threshold = int(alerts_payload.get("runtime_threshold_seconds", 3600) or 3600)
        runtime_breach = bool(alerts_payload.get("runtime_breach", False))
        policy_changed = bool(alerts_payload.get("default_policy_changed", False))
        policy_changes = alerts_payload.get("default_policy_changes") if isinstance(alerts_payload.get("default_policy_changes"), list) else []

        lines.extend(["", "Alerts"])
        lines.append(
            f"- Runtime: {runtime_seconds}s (threshold {runtime_threshold}s)"
            + (" [breach]" if runtime_breach else "")
        )
        lines.append(f"- Default policy changed: {'yes' if policy_changed else 'no'}")

        if policy_changed and policy_changes:
            lines.append("")
            lines.append("Default policy diff (before -> after)")
            for change in policy_changes[:20]:
                field = str(change.get("field", ""))
                before = change.get("before")
                after = change.get("after")
                lines.append(f"- {field}: {before} -> {after}")
            if len(policy_changes) > 20:
                lines.append(f"- ... and {len(policy_changes) - 20} more changes")
...
def main() -> None:
    ...
    if alerts_path.is_file():
        try:
            parsed = json.loads(alerts_path.read_text(encoding="utf-8"))
            if isinstance(parsed, dict):
                alerts_payload = parsed
        except Exception:
            alerts_payload = {}
